...
router.get("(/status/:status)?", async (req, res, next) => {
	...
	let statusFilter = await helperPublish.createFillterStatus(
	...
	params.paginaTion.currentPage = parseInt(
		paramHelper.getParam(req.query, "page", 1)
	);
	...
	uploadThum(req, res, async (err) => {
		...
		if (typeof item !== "undefined" && item.id !== "") {
			if (errors) {
				res.render(
					`${folderView}/form`,
					{ pageTitle: "edit Items", item, errors, categoryItems },
					CategoryModel
				);
			} else {
				if (req.file == undefined) {
					// if not user not reupload avatar
					item.thumb = item.dfImg;
				} else {
					// reUpload avatar and delete old avatar
			...
			if (errors) {
				res.render(
					`${folderView}/form`,
					{ pageTitle: "edit Items", item, errors },
					CategoryModel
				);
			} else {
				item.thumb = req.file.filename;
				ArticleModel.saveItem(item, { task: "add" }).then(() => {
					...
					res.render(
						`${folderView}/form`,
						{ pageTitle: "add Items", item, errors },
						CategoryModel
					);
				});
			}
